      ...
      <div className="shrink-0 px-3 py-2.5 border-b border-white/10">
        <div className="flex items-center justify-between gap-2 mb-2">
          ...
          <button
            ...
            className="p-2 rounded-lg hover:bg-white/10 active:bg-white/20 text-white/60 transition"
            title="New message"
          >
            <Icon name="Pencil" className="w-4 h-4" />
          </button>
        </div>
        <div className="flex items-center gap-2 px-2.5 py-2 rounded-lg bg-white/10 focus-within:bg-white/15">
      ...
      <div className="flex-1 overflow-y-auto min-h-0 overscroll-contain">
        {filtered.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-12 px-4 text-white/40 text-center">
            <p className="text-sm">{search.trim() ? "No matches found." : "No conversations yet."}</p>
          </div>
        ) : (
          <ul className="py-1">
              ...
              return (
                <li key={preview.threadId}>
                  <button
                    ...
                    className={`w-full flex items-center gap-2.5 px-3 py-2.5 text-left transition text-sm active:scale-[0.98] ${
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      {snippet && (
                        <p className={`text-xs truncate mt-0.5 ${isSelected ? "text-white/70" : "text-white/40"}`}>{snippet}</p>
                      )}
                    </div>
                  </button>
                </li>
